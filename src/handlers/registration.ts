...
export async function handleStart(ctx: MyContext) {
  try {
    ...
    await ctx.reply("Ой-ой! Что-то пошло не так... Похоже, кого-то сегодня уволят..");
  }
}
...
export async function registrationConversation(
  ...
  try {
    await ctx.reply(
      "Добро пожаловать!\nУчаствуя в проекте, Вы подтверждаете, что ознакомились с правилами проекта из канала @podarivalentinku.\nПомните, что относиться ко всем стоит уважительно!",
    );
    await ctx.reply(
